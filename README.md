...
Tablet-mode automation depends on Niri switch events. Add or adapt the following block in your Niri config:
...
The plugin will save your normal bar density, switch to the configured tablet density when Niri reports tablet mode, and restore the previous density when tablet mode ends.
...
If auto-rotate is enabled, the plugin also starts `monitor-sensor --accel` while tablet mode is `on`, maps the reported orientation to Niri transforms, and restores the previous transform when tablet mode ends. Auto-rotate only targets the internal panel:
...
- Rotation is applied at runtime through Niri and is not a permanent display configuration.
- If output configuration is later reloaded from disk, the temporary transform may be lost.
- Tablet-mode gating still comes from the Niri `switch-events` state file; no additional Niri config is required beyond the block above.
